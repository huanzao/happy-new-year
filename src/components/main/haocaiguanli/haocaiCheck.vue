<template>
    <div>
        ...
        <div class="pageHead">
            ...
            <!-- <el-button size="mini" @click="myAdd()"  type="primary">申请报废</el-button> -->
            <el-button size="mini" @click="addCheck()"  type="primary">新增盘点计划</el-button>
            <el-button size="mini" @click="exportFile()"  type="primary">导出</el-button>
        </div>
        <!-- 筛选 -->
        <div class="shaixuan">
            <div>
                <label>厂商名称：</label>
                <div>
                     <el-select v-model="shaixuan.facData.factoryname" filterable placeholder="请选择" size="mini">
                        <el-option
                                v-for="item in shaixuan.facData"
                                :key="item.factoryid"
            ...
            <div>
                <label>启用状态：</label>
                <div>
                    <el-select v-model="shaixuan.status.name" filterable placeholder="请选择" size="mini">
                    <el-option
                        v-for="item in shaixuan.status"
                        :key="item.id"
                        :label="item.name"
                        :value="item.id">
                        </el-option>
        ...
        <div class="block" style="height:32px">
            <el-pagination
            style="height:50px"
              @size-change="handleSizeChange($event,'api/haocaicheck/lookcheck')"
              @current-change="handleCurrentChange($event,'api/haocaicheck/lookcheck')"
...
export default {
    data() {
      return {
        shaixuan:{
            name:'',
            type:'',
            status:[{id:0,name:"禁用"},{id:1,name:"启用"}],
            facData:[]
        },
        peopleData:[],
        ksData:[],
    ...
    mounted(){
        this.myInit('api/haocaicheck/lookcheck',{IndexPage:"1",PageSize:"10"})
    ...
    methods:{
        ...
        detail(row){  
            ...
            this.axios({
                ...
                headers:{'Authorization':'Bearer '+localStorage.token},
                timeout:5000,
                url: 'api/haocaicheck/checkdetail',
                data:this.$qs.stringify(params)
            }).then((res)=>{
                loading.close()
        ...
        addCheck(){
            ...
            this.drawer_title="新增耗材盘点计划";

        },
        writeCheck(row){
            this.drawer=true;
            this.drawer_title="耗材盘点录入"
            this.ruleForm1.plan_id=row.id
            this.ruleForm1.status=row.status
            var params={plan_id:''}
            params.plan_id=row.id;
        ...
        check(row){
         ...
         upload(type){ 
             if(type=="1"){
               this.addUpload_current('api/haocaicheck/addcheck','ruleForm2')
             }else if(type=="2"){
               this.addUpload_current('api/haocaicheck/shenpicheck','ruleForm')
             }
        ...
        submitData(){
            ...
            this.axios({
                ...
                headers:{'Authorization':'Bearer '+localStorage.token},
                timeout:5000,
                url: 'api/haocaicheck/writecheck',
                data:this.$qs.stringify(params)
            }).then((res)=>{
                loading.close()
